...
- 💰 Elastic IPを含むパブリックIPv4は、使用中・未使用とも通常料金が発生します。使い終わったら必ず「アドレスの解放」まで行いましょう。

## 💰 コスト概算

| 項目 | 無料枠・料金の目安 | 注意点 |
|---|---|---|
| EC2 (t2.micro / t3.micro) | リージョン・タイプ・稼働時間で見積もる | 無料プラン・クレジットの適用条件を確認 |
| EBSストレージ(EC2に取り付ける仮想ディスク) | 容量・種類・利用期間で見積もる | EC2停止中や削除後も、残ったボリュームは別途確認 |
| Elastic IP | 使用中・未使用とも通常料金0.005 USD/アドレス/時間（2026-09-17確認） | クレジット等の適用前。終了後は解放まで確認 |
| データ転送(アウト) | 転送元・先、量に応じた条件を確認 | 無料になる範囲を決めつけない |

無料利用枠は2025-07-15を境にアカウント作成日によって制度が異なります。[EC2の公式条件](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/ec2-free-tier-usage.html)、[VPC料金](https://aws.amazon.com/vpc/pricing/)、[共通コストガイド](../../docs/03-cost-management.md)を確認してください。
