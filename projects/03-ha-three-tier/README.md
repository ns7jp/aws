...
7. プライベートAPP用のルートテーブル(`ha-private-app-rt`)を作成し、送信先 `0.0.0.0/0` → ターゲットに手順6のNATゲートウェイを指定するルートを追加。private-app-1a・private-app-1cを関連付けます(DB用サブネットにはインターネットへの経路は不要なので関連付けません)。
   ...
   ⚠️ **設計上のトレードオフ**: 今回はコスト削減のため、NATゲートウェイをpublic-1a側の1つだけに構築しています。そのため「ALB経由の利用者アクセス」自体は両AZのEC2が生きていれば継続しますが、AZ1aに障害が起きた場合、AZ1c側のEC2はOSアップデートなどのアウトバウンド通信ができなくなります。真の意味で「NAT部分までAZ障害に強くする」には、AZごとにNATゲートウェイを1つずつ(計2つ)用意し、各AZのプライベートルートテーブルをそれぞれ同一AZのNATゲートウェイに向ける構成が必要です。
...
| RDSリードレプリカ(発展課題、既定では無効) | 通常のRDSインスタンス1台分の料金が追加でかかる | `ENABLE_READ_REPLICA=true`で有効化した場合のみ発生。検証後は`cleanup.sh`で忘れずに削除する |
...
- **クロスゾーン負荷分散**: ALBでは常時有効(無効化する設定項目自体が無い)で、複数AZのターゲットへ自動的に均等分散されます。NLB(Network Load Balancer)では既定で無効・任意で有効化できる点との違いを比較して理解しておくと、面接で「なぜALBではこの設定を意識しなくてよいのか」まで説明できます。
- ✅ **スティッキーセッション**: `handson/build.sh`実行時に`ENABLE_STICKY_SESSION=true`を指定すると、ターゲットグループでlb_cookie方式のスティッキーセッション(有効期限24時間)が有効化されます。同じ利用者からの通信を毎回同じEC2に固定し、ログインセッションを保持するアプリでどう挙動が変わるかを検証できます。
- ✅ **RDSリードレプリカの追加**: `handson/build.sh`実行時に`ENABLE_READ_REPLICA=true`を指定すると、読み取り専用の複製インスタンス(`<DB識別子>-replica`)が追加されます。リードレプリカの作成にはソース側の自動バックアップ有効化が必須のため、スクリプトが既定の`backup-retention-period 0`を一時的に`1`へ引き上げてから作成します。Multi-AZ(可用性目的の待機系)とリードレプリカ(性能拡張目的の読み取り分散)の役割の違いを体感できます。`cleanup.sh`はリードレプリカをプライマリより先に削除します(プライマリはレプリカが存在する間は削除できないため)。
