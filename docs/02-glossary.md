...
| **Apache** | 世界中で長年使われてきた、代表的なオープンソースのWebサーバーソフトです。Linux上でのサービス名・コマンド名は `httpd` です | [⑨Linux基礎](glossary/09-linux-server-basics.md#apache) |
...
| **CNAMEレコード** | あるドメイン名を、別のドメイン名の「別名」として転送するレコードです | [②ネットワーク](glossary/02-network.md#cnameレコード) |
...
| **CodeStar Connections** | GitHubなど外部のコードリポジトリと、AWSのサービスを安全につなぐための認証済み接続機能です | [⑧IaC/CI-CD](glossary/08-iac-cicd.md#codestar-connections) |
