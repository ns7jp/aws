...
- ✅ 監視は「作って終わり」にせず、**通知が実際に届くところまで必ずテスト**します。アラームの詳細画面から手動で `ALARM` 状態に変更する(CLIの `aws cloudwatch set-alarm-state`)と、本物の障害を起こさずに通知経路を確認できます。
...
**もう少し詳しく**: 点検の観点は、コスト最適化・パフォーマンス・セキュリティ・耐障害性・サービスクォータ(上限値)などのカテゴリに分かれています。たとえば「稼働率の低いEC2インスタンスがある」「アタッチされていない[Elastic IP](02-network.md#elastic-ip)がある」「[セキュリティグループ](02-network.md#セキュリティグループ)で特定ポートが全世界に開放されている」「[MFA](06-security-identity.md#mfa)が有効になっていないルートユーザーがある」といった指摘が並びます。**利用できるチェック項目はサポートプランによって異なり**、無料のベーシックプランではサービスクォータと一部のセキュリティチェックに限定されます。ビジネス以上のプランでは全チェックが使えるようになり、[EventBridge](#eventbridge)経由での通知やAPI経由の取得も可能になります。
...
**もう少し詳しく**: AWSサービスが発行するメトリクスの名前空間は `AWS/` で始まる決まった名前になっています。代表例は、EC2が `AWS/EC2`、RDSが `AWS/RDS`、ALBが `AWS/ApplicationELB`、Auto Scalingが `AWS/AutoScaling`、CloudWatch Logsが `AWS/Logs`、請求が `AWS/Billing` です。**名前空間が違えばメトリクスは完全に別物**として扱われ、名前空間をまたいだ集計はできません(比較したい場合はダッシュボードのグラフに重ねるか、メトリクス計算で式を組みます)。自分でメトリクスを送るときは `AWS/` で始まらない任意の名前(たとえば `CWAgent` や `MyApp/Web`)を付けます。[CloudWatchエージェント](#cloudwatchエージェント)が既定で使う名前空間は `CWAgent` です。
...
- ✅ ディメンションを指定せずにメトリクスを検索すると、「全インスタンスを合算した値」ではなく「ディメンションなしのメトリクス(通常は存在しない)」を探すことになります。コンソールのメトリクス画面では、必ずディメンションの階層をたどって目的の1行を選んでください。
...
**もう少し詳しく**: 期間は60秒(1分)の倍数で指定するのが基本で、5分(300秒)、15分(900秒)などがよく使われます。1分未満(1秒・5秒・10秒・30秒)を指定できるのは**高解像度メトリクス**として送信されたものに限られ、その場合はアラームの単価も変わります。ここで重要なのは、**元のメトリクスの発行間隔より短い期間を指定しても意味がない**ということです。EC2の標準メトリクスは既定で5分間隔なので、期間を1分にすると「データがある1分」と「データがない4分」が交互に並び、アラームが不安定になります。1分間隔で監視したい場合は[詳細モニタリング](#詳細モニタリング)を有効にします。逆に[EstimatedCharges](#estimatedcharges)のように**数時間に一度しか更新されない**メトリクスでは、期間を6時間(21600秒)程度に取るのが定石です。
...
**サーバー構築での勘所**:
- ⚠️ `t` 系の**バースト可能インスタンス**では、CPU使用率が低いのに動作が遅くなることがあります。CPUクレジット(平常時に貯めておき、高負荷時に使う性能の預金)を使い切ると性能に上限がかかるためで、この場合は `CPUCreditBalance` を併せて監視します。CPU使用率だけを見ていると原因にたどり着けません。
...
**もう少し詳しく**: オンコール体制の設計要素は、**当番表(ローテーション)**、**通知手段**(メールでは寝ている人を起こせないので、電話やプッシュ通知を使う)、**エスカレーション**(一次担当が一定時間応答しなければ二次担当へ、さらに上長へ)、そして**引き継ぎ**(当番交代時に、進行中の事象や気になっている兆候を伝える)です。AWSの[SNS](#sns)は通知の入口として使えますが、当番表の管理やエスカレーションといった機能は持っていないため、実務では専用のインシデント管理サービス(AWSにもIncident Managerがあります)やチャットツール連携と組み合わせるのが一般的です。
...
| Incident Manager | インシデントの発生から関係者招集・記録までを管理する機能 | 「非常時の招集係」。オンコールの当番表もここで管理する |
