...
**もう少し詳しく**: 名前は「ファミリー + 世代 + 追加の属性 + サイズ」という決まった構造をしています。`t3.micro` なら、`t`が[インスタンスファミリー](#インスタンスファミリー)、`3`が世代、`micro`がサイズです。`m7g.large` のように末尾に付く小文字は追加の属性を表し、`g`はAWS独自プロセッサのGraviton、`a`はAMD、`d`は[インスタンスストア](#インスタンスストア)付き、`n`はネットワーク強化、といった意味を持ちます。サイズは `nano` → `micro` → `small` → `medium` → `large` → `xlarge` → `2xlarge` … と大きくなり、1段階上がるとメモリはおおむね倍になります。[vCPU](#vcpu)の数は `large` 以上では1段階ごとにおおむね倍になりますが、`nano`〜`medium` の小さいサイズでは変わらないことが多く、たとえば `t3.nano` から `t3.medium` まではいずれも2 vCPUで、増えるのはメモリだけです(最新のスペックは[Amazon EC2 インスタンスタイプ](https://aws.amazon.com/jp/ec2/instance-types/)で確認してください)。インスタンスタイプは起動後でも変更できますが、その際はインスタンスの停止が必要です。
...
**もう少し詳しく**: x86系のインスタンスでは、1 vCPUは物理CPUコアのスレッド1本に相当することが一般的です(2 vCPU = 物理1コア相当、という関係になりやすい)。AWS独自プロセッサであるGraviton系のインスタンスでは、1 vCPUが物理1コアに対応します。インスタンスタイプのサイズが1段階上がると、`large` 以上ではvCPU数もおおむね倍になりますが、`nano`〜`medium` の小さいサイズでは変わらないことが多く、たとえば `t3.nano` から `t3.medium` まではいずれも2 vCPUです(増えるのはメモリだけです)。Linuxのサーバー内からは `nproc` コマンドや `/proc/cpuinfo` で確認できます。
...
- `$Latest` / `$Default` は、起動テンプレートの中ではなく**テンプレートを使う側**が指定します。[Auto Scaling Group](#auto-scaling-group)や起動時のAPI呼び出しで、参照するバージョンを具体的な番号にするか、`$Latest`(常に最新)/`$Default`(テンプレート側で既定に指定したバージョン)にするかを選ぶ形です。テンプレート自身が持つのは「どのバージョンを既定にするか」の設定だけです。事故を防ぐ観点では、ASG側の指定を `$Default` にしておき、テンプレート側で既定バージョンを意図的に切り替える運用が安全です。
...
- トークン取得時のホップ数制限(`HttpPutResponseHopLimit`)は、AMI側にIMDSv2の指定がなければ既定1ですが、Amazon Linux 2023のようにAMIが `imds-support: v2.0` を持つ場合は既定2になります。EC2上のDockerコンテナからメタデータを取りたい場合は2以上が必要なので、実際の値を確認してください。
...
**サーバー構築での勘所**:
- 検証が終わったら、停止ではなく**終了**まで行うのがコスト事故を防ぐ基本です。停止したまま放置すると、EBSと、割り当てられているパブリックIPv4アドレス([Elastic IP](02-network.md#elastic-ip)を含む)で課金が続きます。2024年以降、パブリックIPv4アドレスはアタッチ済みかどうかにかかわらず時間課金の対象です。削除の順番は[コスト管理ガイド](../03-cost-management.md)にまとめています。
...
**💰 コスト**: 停止中もEBSは課金対象で、割り当てたままのパブリックIPv4アドレス([Elastic IP](02-network.md#elastic-ip)を含む)の時間課金も続きます。「止めたから0円」ではない点に注意してください。
...
**もう少し詳しく**: 主な方式は4つあります。(1)**ターゲット追跡**は「平均CPU使用率70%を保つ」のように目標値だけを決める方式で、増減量はAWSが計算します。もっとも簡単で、推奨される第一候補です。(2)**ステップスケーリング**は「70%超で1台、90%超で3台」のように、超過の度合いに応じた段階的な増減を自分で定義する方式です。(3)**簡易スケーリング**は古い方式で、1つのアラームに対して1つの動作を割り当てます。(4)**予測スケーリング**は、過去の負荷パターンから将来の需要を機械学習で予測し、負荷が来る前にあらかじめ台数を増やしておく方式です。毎日・毎週決まった波があるシステムで、ターゲット追跡と併用して使います。これらとは別に、時刻を指定して台数を変える「スケジュールされたスケーリング」もあり、「平日9時に増やし、22時に減らす」といった予測可能な波に有効です。
...
**もう少し詳しく**: 「サーバーがない」わけではなく、「利用者がサーバーを管理しなくてよい」という意味です。OSのパッチ、台数の調整、障害時の入れ替えはすべてAWSが担当します。特徴は、(1)使った分だけの従量課金で、リクエストが無いときの費用を抑えやすい、(2)必要に応じて自動的に並列実行される、(3)OSレベルの設定はできない、の3点です。ただし課金の軸はサービスによって違い、[Lambda](#lambda)のように待機中はまったく課金されないものもあれば、[Fargate](#fargate)のようにタスクが起動している間はずっと課金されるものもあります。「サーバーレス=動いていなければ0円」と単純に覚えないでください。代表格が[Lambda](#lambda)で、ほかにも[Fargate](#fargate)、Amazon S3、DynamoDBなど「サーバーを意識しないサービス」がこの系統に含まれます。[責任共有モデル](01-cloud-basics.md#責任共有モデル)でいえば、利用者の責任範囲がもっとも狭くなる方式です。
...
| ステータスチェック | AWSが行う、インスタンス・システム・アタッチ済みEBSの死活確認 | 「部屋の電気は点いているか(インスタンス)」「建物の電源は来ているか(システム)」「倉庫に出入りできるか(アタッチ済みEBS)」の3種類 |
...
| 使っていないのにEC2の料金が発生し続けている | 停止ではなく終了していない、[最小キャパシティ](#最小キャパシティ)が0になっていない | 検証後はASGごと削除するか最小・希望を0にし、EBSとElastic IP(アタッチ中も課金対象)も解放する([コスト管理ガイド](../03-cost-management.md)) |
