...
**もう少し詳しく**: Amazon Linuxにログインすると、既定ではbashが起動します。実務で頻出するのは次の書き方です。変数展開 `${VAR}`、コマンド置換 `$(コマンド)`、パイプ `コマンドA | コマンドB`、リダイレクト `> file`(上書き)と `>> file`(追記)。たとえばレベル3のユーザーデータにある `TOKEN=$(curl -s -X PUT "http://169.254.169.254/latest/api/token" -H "X-aws-ec2-metadata-token-ttl-seconds: 21600")` は、「curlの実行結果を変数に入れる」というコマンド置換そのものです。なお Amazon Linux 2023 は既定で[IMDSv2](03-compute.md#imdsv2)必須のため、インスタンスメタデータを取得するときは、このようにトークンの取得を先に行う必要があります。このポートフォリオのハンズオンキット(`build.sh` / `cleanup.sh`)もbashスクリプトで書かれています。
...
- SSHの鍵ファイルは厳格です。秘密鍵は `600`(または `400`)、`~/.ssh` ディレクトリは `700` にします。手元の秘密鍵が緩いとSSHクライアントが鍵の使用を拒否し、サーバー側の `~/.ssh` や `authorized_keys` が緩いと[sshd](#sshd)が公開鍵認証を拒否します。
...
**正式名称**: change mode ｜ **読み方**: チモド/チェンジモード ｜ **重要度**: ★★★

**ひとことで言うと**: ファイルやディレクトリのパーミッション(権限)を変更するコマンドです。

**たとえるなら**: 部屋の入室ルールを書き換える手続きです。「この部屋は本人だけ」「この部屋は誰でも閲覧可」と札を付け替えるイメージです。

**もう少し詳しく**: 数字で指定する方法(`chmod 600 key.pem`)と、記号で指定する方法(`chmod u+x script.sh` = 所有者に実行権限を追加)の2通りがあります。実務で最初に必ず出会うのが、レベル2の手順にある `chmod 400 handson-key.pem` です。これは「所有者だけが読める、書き込みも実行もできない」状態にする操作で、これを行わないと**手元のSSHクライアント**が「鍵ファイルの権限が緩すぎる(`WARNING: UNPROTECTED PRIVATE KEY FILE!`)」と判断し、その鍵を使わずに接続を中止します(サーバー側の[sshd](#sshd)も、サーバー上の `~/.ssh` や `authorized_keys` の権限が緩い場合に同様のチェックを行います)。鍵ファイルは他人に読めた時点で価値を失うため、SSHはあえて厳しくチェックしているのです。
...
**ひとことで言うと**: 世界中で長年使われてきた、代表的なオープンソースのWebサーバーソフトです。Amazon LinuxなどRed Hat系Linuxでのサービス名・コマンド名は `httpd`(Debian/Ubuntu系では `apache2`)です。
...
**サーバー構築での勘所**:
- `wa` が高いのにCPU使用率が低い、という状態はディスクI/Oのボトルネックのサインです。[EBS](04-storage.md#ebs)のボリュームタイプや、[gp3](04-storage.md#gp3)であればIOPS・スループットの指定値の見直しを検討します(gp3は容量と性能が独立しているため、容量を増やしても性能は上がりません。旧世代のgp2では容量に比例して性能が決まります)。
