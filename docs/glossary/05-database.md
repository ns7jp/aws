...
flowchart TD
    ...
    Cache -.->|"2. 早見表に無ければ金庫室へ"| EP
    App -->|"3. 書き込みは必ず代表番号あて"| EP
    EP --> Pri
    Pri -->|"4. 同期レプリケーション(可用性)"| Std
    Pri -->|"5. 非同期レプリケーション(性能)"| RR
    App -.->|"6. 参照だけならこちらへ逃がす"| RR
    Pri -->|"7. 毎日決まった時刻に取得"| Bkup
    App -.->|"8. 用途が合えば別の選択肢"| Dyn
```
...
**もう少し詳しく**: 1つのDBインスタンスの中には、複数のデータベース(スキーマ)を作れます。たとえばWordPress用とアプリ用のデータベースを同じインスタンスに同居させることも可能です。DBインスタンスには識別子(DBインスタンス識別子)という名前を付け、これがそのまま[DBエンドポイント](#dbエンドポイント)の先頭部分になります。EC2の[インスタンス](03-compute.md#ec2)と考え方は似ていますが、OSにログインできない点、AZの指定やストレージの扱いがRDS独自である点が異なります。

**サーバー構築での勘所**:
- 開発環境と本番環境で同じDBインスタンスを共用しないでください。設定変更や再起動が互いに影響し、切り分けが困難になります。環境ごとにDBインスタンスを分けるのが基本です。
- ⚠️ 削除時に「最終スナップショットを作成しますか」と聞かれます。学習用リソースの後片付けであれば不要ですが、業務環境では**必ず作成**してください。削除と同時に[自動バックアップ](#自動バックアップ)も消えるため、最終スナップショットが唯一の命綱になります。
...
**サーバー構築での勘所**:
- ⚠️ `t` 系はCPUクレジット(貯めておいて必要なときに使う性能の権利)を使い切ると性能が大きく落ちます。学習・検証には最適ですが、常時高負荷がかかる本番環境では `m` 系や `r` 系を検討してください。「テスト環境では速かったのに本番で遅い」の典型的な原因です。
...
- [CloudWatch](07-monitoring-operations.md#cloudwatch)でCPU使用率・空きメモリ・空きストレージ・DB接続数を監視するときは、プライマリだけでなくスタンバイやレプリカも監視対象に含めます。[レベル4](../../projects/04-wordpress-production/README.md)では `FreeStorageSpace` が2GiB未満で通知する[しきい値](07-monitoring-operations.md#しきい値)を設定しています。
...
**サーバー構築での勘所**:
- ⚠️ プロモート後は[DBエンドポイント](#dbエンドポイント)が別のものになります。アプリの接続先を切り替える作業が必ず発生する、という点を復旧手順書([ランブック](07-monitoring-operations.md#ランブック))に明記しておいてください。
...
**サーバー構築での勘所**:
- ⚠️ **DBインスタンスを削除すると、自動バックアップも一緒に消えます。** 残したいなら[DBスナップショット](#dbスナップショット)を手動で取るか、削除時に最終スナップショットを作成してください。この仕様を知らずにデータを永久に失う事故が実際に起きています。
...
| 8 | 自動バックアップとDBスナップショットの最大の違いは何ですか | 寿命です。自動バックアップはDBインスタンス削除時に消えますが、手動スナップショットは明示的に削除するまで残ります |
