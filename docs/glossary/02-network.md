...
flowchart TD
    ...
    subgraph VPCBox["VPC 10.0.0.0/16<br/>自分専用の敷地"]
        ...
        subgraph Pri["プライベートサブネット 10.0.11.0/24<br/>奥の事務所棟"]
            EC2N["EC2 Webサーバー(棟の中の1部屋)<br/>セキュリティグループ(個人のIDカード)が付く"]
        end
    ...
    IGW --> RT --> ALB
    NACL -.->|"関連付けたサブネットを検査"| Pub
    NACL -.->|"関連付けたサブネットを検査"| Pri
    ALB -->|"5. HTTP 80番で振り分け"| EC2N
...
**よくあるつまずき**: 「サーバーを停止して起動し直したらアクセスできなくなった」の多くは、自動割り当てのパブリックIPアドレスが変わったことが原因です。ここは正確に区別してください。OSの再起動(reboot。インスタンスは動かしたまま中のOSだけを立ち上げ直すこと)ではアドレスは変わりません。アドレスが解放されて別の番号に変わるのは、インスタンスを「停止(stop)」してから「開始(start)」したときです。固定したい場合は[Elastic IP](#elastic-ip)を使います。
...
**もう少し詳しく**: 「パブリック」という属性のスイッチがあるわけではありません。そのサブネットに関連付けたルートテーブルに `0.0.0.0/0` → [インターネットゲートウェイ](#インターネットゲートウェイ) というルートが書かれていれば、それがパブリックサブネットです。ここに置くのは、外部からの通信を最初に受ける[ALB](#alb)、外向き通信の出口となる[NATゲートウェイ](#natゲートウェイ)、そして踏み台サーバー(bastion。プライベートサブネットのサーバーへ入るために、いったん経由する中継用のサーバー)など、外部との接点になるリソースに限定します。アプリケーションサーバーやデータベースは置きません。
...
- VPCコンソールから「VPCを削除」を実行すると、IGW・サブネット・ルートテーブル・ネットワークACLなどは一緒に削除されます。一方、EC2・RDS・NATゲートウェイ・ALB・それらが残した[ENI](#eni)は自動では消えず、残っていると「依存関係エラー」で削除に失敗します。片付けは「中身(EC2・RDS)→出口(NATゲートウェイ・ALB)→VPC」の順が安全です。
...
**よくあるつまずき**: NATゲートウェイをプライベートサブネットに作ってしまうと、当然ながら外に出られません。置き場所は必ずパブリックサブネットです。なお、Elastic IPを持たない「プライベートNATゲートウェイ」という種類もありますが(オンプレミスや他のVPCとの通信でIPの重複を避ける用途です)、学習段階で使うのは上で説明したパブリック型です。
...
**もう少し詳しく**: EC2を停止・起動するとパブリックIPは変わってしまいますが、Elastic IPを関連付けておけばアドレスは変わりません。また、障害時に別のインスタンスへEIPを付け替えることで、同じアドレスのままサービスを継続する使い方もできます。[NATゲートウェイ](#natゲートウェイ)(パブリック型)にも1つ必要です。なお、Webサービスの公開自体は[ALB](#alb)や[CloudFront](#cloudfront)+[Route 53](#route-53)で行うのが現代的で、EIPが必須になる場面は「固定IPを外部に申告する必要があるとき」に絞られてきています。

**サーバー構築での勘所**:
- アカウントごとにリージョン単位でEIPの数に上限(既定のクォータ)があります。使わなくなったEIPは解放しておきます。
- 外部サービスのIP許可リストに登録してもらう場合は、EIPを使うと申請先に変更を依頼せずに済みます。

**💰 コスト**: ⚠️ 未使用のまま保持しているEIPには課金されます。さらに2024年以降は、パブリックIPv4アドレスに対して使用中かどうかにかかわらず時間課金が発生する体系になりました(「アタッチ中は無料」というのは2024年より前の仕様です)。最新の条件は[Amazon VPCの料金](https://aws.amazon.com/jp/vpc/pricing/)と[AWS無料利用枠](https://aws.amazon.com/jp/free/)で必ず確認してください。
...
**もう少し詳しく**: 2種類あります。ゲートウェイ型はS3とDynamoDB専用で、[ルートテーブル](#ルートテーブル)にエンドポイント宛ての経路を追加して使い、追加料金はかかりません。インターフェイス型はサブネット内に[ENI](#eni)を作る方式で、[Systems Manager](07-monitoring-operations.md#systems-manager)や[Secrets Manager](06-security-identity.md#secrets-manager)をはじめ非常に多くのAWSサービスに対応します(S3もインターフェイス型に対応しています)が、時間課金とデータ処理課金が発生します。VPCの中からS3を使うだけであれば、追加料金のかからないゲートウェイ型を選ぶのが基本です。インターフェイス型は[AWS PrivateLink](#aws-privatelink)という技術の上に成り立っています。
...
**もう少し詳しく**: ターゲットグループには、EC2インスタンス、IPアドレス、Lambda関数(ALB専用)、別のALB(NLB専用)などを登録できます。登録できる種類はロードバランサーの種類ごとに決まっており、ALBのターゲットグループに別のALBを登録することはできません。プロトコルとポート(通常はHTTP:80)、そして[ヘルスチェック](#ヘルスチェック)の設定はこのターゲットグループ単位で行います。[Auto Scaling Group](03-compute.md#auto-scaling-group)と紐づけておくと、増えたインスタンスは自動的にターゲットグループへ登録され、減るインスタンスは自動的に登録解除されます。ALBは[リスナールール](#リスナールール)の条件に応じて、どのターゲットグループへ転送するかを決めます。
...
- CloudFrontの背後にALBを置く構成でも、CloudFrontからALBへの通信はインターネット経由になるため、従来はインターネット向けを選び、ALB側でCloudFront経由の通信だけを受け付ける工夫(マネージドプレフィックスリストやカスタムヘッダーの検証)を加えるのが定石でした。
- なお2024年以降は CloudFront VPC origins という機能があり、プライベートサブネットに置いた内部ALBをそのままCloudFrontのオリジンにできます。ALBをインターネットに一切さらさない構成が組めるので、新規設計ではこちらも検討します(対応状況や条件は[Amazon CloudFront デベロッパーガイド](https://docs.aws.amazon.com/AmazonCloudFront/latest/DeveloperGuide/Introduction.html)で確認してください)。
...
**ひとことで言うと**: あるドメイン名の名前解決を、別のドメイン名に委ねる(別名を張る)レコードです。URLが書き換わるHTTPリダイレクトとは別物で、`example.com` のようなゾーン頂点には設定できません(そこではエイリアスレコードを使います)。

**たとえるなら**: 電話帳の「旧社名の欄」に「新社名の欄を見てください」と書いてある案内です。案内を読んだ人は新社名の欄で電話番号を調べます。調べる人が最終的に手にするのは電話番号(IPアドレス)であって、看板の名前が書き換わるわけではありません。
...
| 踏み台サーバー | プライベートサブネットのサーバーへ入るために、いったん経由する中継用サーバー(bastion) | 奥の部屋へ行く前に必ず通る「管理人室」 |
